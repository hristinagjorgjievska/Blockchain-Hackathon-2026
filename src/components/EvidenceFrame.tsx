...
function SceneFrame({ violation }: FrameProps) {
  ...
  return (
    <svg viewBox={VIEWBOX} style={{ width: '100%', height: 'auto', display: 'block' }} role="img">
      ...
      {violation.kind === 'bus_lane' && (
        <polygon points="480,344 496,362 480,380 464,362" fill="#ffffff" opacity={0.82} />
      )}
      ...
      <text x={92} y={27} fill="#ffffff" fontSize={16} fontWeight={800} dominantBaseline="central">
        SAFE CITY MK
      </text>
      <text x={232} y={27} fill="#9fb3c8" fontSize={13} dominantBaseline="central">
...
function PlateFrame({ violation }: FrameProps) {
  ...
  return (
    <svg viewBox={VIEWBOX} style={{ width: '100%', height: 'auto', display: 'block' }} role="img">
      ...
      <rect x={-40} y={196} width={1040} height={258} rx={46} fill={`url(#carGrad-${uid})`} />
      <circle cx={70} cy={300} r={70} fill="#e64b3c" opacity={0.4} filter={`url(#glow-${uid})`} />
      <circle cx={890} cy={300} r={70} fill="#e64b3c" opacity={0.4} filter={`url(#glow-${uid})`} />
      <rect x={-80} y={262} width={250} height={86} rx={22} fill={`url(#tailGrad-${uid})`} opacity={0.92} />
      <rect x={790} y={262} width={250} height={86} rx={22} fill={`url(#tailGrad-${uid})`} opacity={0.92} />
      <rect x={-40} y={244} width={1040} height={40} fill="#17191e" opacity={0.92} />

      {plateGroup(220, 300, 520, violation.plate)}

      {cornerBrackets(196, 286, 568, 142, 46, CYAN)}
      <line x1={210} y1={356} x2={750} y2={356} stroke={CYAN} strokeWidth={2.5} opacity={0.85}>
        <animate attributeName="y1" values="296;420;296" dur="3.4s" repeatCount="indefinite" />
        <animate attributeName="y2" values="296;420;296" dur="3.4s" repeatCount="indefinite" />
      </line>
