...
import { canonicalString, findViolation } from '../data/violations';
...
export function ViolationView({ code, onBack }: { code: string; onBack: () => void }) {
  const { t, lang } = useLang();
  const violation = useMemo(() => findViolation(code), [code]);
    ...
    <div className="mx-auto max-w-6xl px-4 py-6">
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3">
        ...
        <div className="ml-auto rounded-xl bg-navy px-4 py-2 text-right text-white">
          ...
          <div className="text-lg font-extrabold">{formatMKD(violation.amountDueMKD, lang)}</div>
        </div>
      </div>
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
            ...
            <div className="mt-4 grid gap-4 sm:grid-cols-2">
              ...
              <Field
                ...
                value={formatDate(violation.dueDate, lang)}
              />
            </div>
          </section>
